...
async def process_getsms(update: Update, context, message):
    ...
    if search_value:
        ...
        try:
            ...
            conn.close()
async def send_document(update: Update, context: ContextTypes.DEFAULT_TYPE, file_path: str, caption: str):
    try:
        with open(file_path, 'rb') as document:
            await context.bot.send_document(chat_id=update.message.chat_id, document=document, caption=caption)
        await update.message.reply_text(f'The document has been sent to you.')
    except Exception as e:
        logging.error(f"Error sending document: {str(e)}")
        await update.message.reply_text("Oops! Something went wrong while sending the document.")
